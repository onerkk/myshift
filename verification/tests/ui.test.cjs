...
test('salary shows configured amounts and only claims verification when the difference is zero',()=>{
  const c=env();let h=c.uiSalaryDashboardHtml(2026,8);assert.match(h,/\$55,344/);assert.match(h,/2026\/07\/26 – 2026\/08\/25/);assert.match(h,/扣假後工時/);
...
test('company slip and forecast remain separately labelled on payroll and home screens',()=>{
  const c=env();const slip={baseSum:39590,proposal:800,otherIncome:0,otTaxFree:8688,otTaxable:2200,holidayPay:5664,nightPay:3525,fixedDed:2540,leaveDed:4289,laborPensionSelf:0,income:60467,deduction:6829,net:53638,weekdayH:44,holidayH:19,sickH:52,disasterH:12,payDate:'2026-09-04'};
  Object.assign(c.estimate,{baseSum:39590,otPay:15812,holidayPay:0,nightPay:6083,proposal:0,otherIncome:0,fixedDed:2540,leaveDed:3601,laborPensionSelf:0,hasSlip:true,official:slip,notes:[]});
  c.estimate.reconciliation=c.Payroll.reconcile(c.estimate,slip);
  const h=c.uiSalaryDashboardHtml(2026,8);assert.match(h,/公司實領 · 薪資條記錄/);assert.match(h,/\$53,638/);assert.match(h,/班表估算實領/);assert.match(h,/\$55,344/);assert.match(h,/\+\$1,706/);assert.match(h,/假日加班費/);assert.match(h,/公司薪資明細/);assert.doesNotMatch(h,/已核對實領|分項一致|undefined|NaN/);
  const home=c.uiPayPreviewHtml();assert.match(home,/8 月公司實領/);assert.match(home,/\$53,638/);
});
