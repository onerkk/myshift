...
function _syncAnnualToALD(){if(!fbUser)return;let changed=false;for(const date in leavesCache){let h=0;leavesCache[date].forEach(l=>{if(l.uid===fbUser.uid&&l.leaveType==="annual")h+=l.hours||0});if(h>0){if(ALD[date]!==h){ALD[date]=h;changed=true}}else if(ALD[date]&&!leavesCache[date].some(l=>l.uid===fbUser.uid&&l.leaveType==="annual")){delete ALD[date];changed=true}}if(changed)sAL()}
