...
function studioWeatherSculpture(code){
  const c=code===null||code===undefined?NaN:Number(code);
  const type=c===0?'clear':c>=95?'storm':c>=51?'rain':'cloudy';
  return `<span class="sky-sculpture sky-${type}" aria-hidden="true"><i class="sky-orbit"></i><i class="sky-sun"></i><i class="sky-cloud"></i><i class="sky-drops"><b></b><b></b><b></b></i><i class="sky-ground"></i></span>`;
}
...
function uiPayPreviewHtml(y,m){
  ...
  const title=actual?(lang==='zh'?'公司實領':'Gaji bersih'):est?salaryForecastTitle(est):(lang==='zh'?'設定自動計薪':'Atur hitungan otomatis');
  return `<button class="insight-row insight-pay" data-a="tabPay"><span class="insight-icon pay">${studioIcon('money',23)}</span><span class="insight-copy"><small>${m} 月 · ${title}</small><strong>${value}</strong></span><span class="insight-meta">${actual?(lang==='zh'?'薪資條記錄':'Slip tersimpan'):(lang==='zh'?'班表自動計算':'Hitungan jadwal')}</span>${uiIcon('chevron',18)}</button>`;
}
...
function uiSalaryDashboardHtml(y,m){
  ...
  if(est.dataPending&&!actual)return `<section class="salary-dashboard">${head}<div class="payroll-auto-badge">${isZh?'薪資紀錄 · 自動核算':'Catatan gaji · hitungan otomatis'}</div><div class="salary-hero payroll-incomplete"><span>${salaryForecastTitle(est)}</span><p>${salaryNoteText('leaveNotReady')}</p><button class="soft-btn" data-a="salRefresh">${isZh?'重新同步':'Sinkronkan lagi'}</button></div></section>`;
...
function rainObsHtml(){
  ...
  const values=[[isZh?'10 分鐘':'10m',m.r10],[isZh?'1 小時':'1h',m.r1],[isZh?'3 小時':'3h',m.r3],[isZh?'24 小時':'24h',m.r24]];
  return `<section class="rain-station rain-${state}"><div class="rain-station-head"><div class="rain-station-icon">${studioIcon(icon,19)}</div><div><strong>${isZh?'即時雨量站':'Live rain station'}</strong><span>${trust}</span></div><b>${m.local?(isZh?'本地實測':'Lokal'):(isZh?'測站參考':'Ref')}</b></div><div class="rain-station-place"><strong>${station}</strong>${town?`<span>${town}</span>`:''}</div><div class="rain-station-meta">${dist?`<span>${isZh?'距離':'Jarak'} ${dist}</span>`:''}<span>${obs}</span></div><div class="rain-metrics">${values.map(([label,value])=>`<div><span>${label}</span><strong>${_rainMm(value)}</strong><small>mm</small></div>`).join('')}</div></section>`;
}
function rainWarnHtml(){
