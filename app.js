...
function _authInit(){
  if(_authInitDone)return;_authInitDone=true;
  loadLeaves();loadAdminEv();cloudLoad();
}
fbAuth.getRedirectResult().then(r=>{
  _redirectChecked=true;
  if(r&&r.user){fbUser=r.user;fbAuthReady=true;_authInit()}
  else if(fbUser){fbAuthReady=true;_authInit()}
  else if(!fbAuthReady){/* wait for onAuthStateChanged */}
}).catch(()=>{_redirectChecked=true;if(fbUser){fbAuthReady=true;_authInit()}});
fbAuth.onAuthStateChanged(u=>{
  fbUser=u;
  if(!_redirectChecked)return;
  fbAuthReady=true;
  if(u){_authInit()}
  else{loadAdminEv();render()}
});
setTimeout(()=>{_redirectChecked=true;if(!fbAuthReady){fbAuthReady=true;render()}},4000);
let _loading=false;
...
fbLoginPending=true;
fbAuth.signInWithPopup(p).then(r=>{fbLoginPending=false;if(r&&r.user){fbUser=r.user;fbAuthReady=true;_authInitDone=false;_authInit()}}).catch(e=>{
...
function fbLogout(){_authInitDone=false;fbAuth.signOut()}
let leavesCache={};
...
if('caches' in window){caches.keys().then(names=>{names.forEach(n=>{if(n!=='myshift-v93')caches.delete(n)})})}
