/* v309 atmospheric renderer. Existing photographic assets; time-based motion.
...
(function(root){
  ...
  root.NatureEffects={classify,create:function(){
    let canvas,sky,ctx,bg,w=0,h=0,raf=0,last=0,time=0,frameCount=0,accum=0;
    let code=null,temp=0,wind=0,mode='none',preview=null,previewUntil=0,quiet=[],rectTick=0;
    ...
    function init(){
      ...
      for(const [key,dir,prefix,count] of [['leaf','maple','maple',4],['cloud','cloud','cloud',3],['butterfly','butterfly','monarch',6],['petal','blossom','blossom',3]]){
        images[key]=Array.from({length:count},(_,i)=>{const image=new Image();image.src='./images/fx/'+dir+'/'+prefix+'-'+String(i+1).padStart(2,'0')+'.png';return image});
      }
      for(let i=0;i<9;i++)leaves.push({x:Math.random(),y:Math.random(),z:rand(.2,1),phase:rand(0,6.28),rot:rand(0,6.28),image:i%4});
      for(let i=0;i<120;i++)rain.push({x:Math.random(),y:Math.random(),z:rand(.15,1)});
      for(let i=0;i<24;i++)stars.push({x:Math.random(),y:Math.random()*.55,r:rand(.35,1.1),phase:rand(0,6.28)});
      resize();addEventListener('resize',resize,{passive:true});addEventListener('scroll',()=>{rectTick=0},{passive:true});
    ...
    function collectQuiet(){
      quiet=[...document.querySelectorAll('.brand-lockup,.header-actions,.hero-countdown,.dial-case,.hero-timeline,.hero-footer,.hero-topline,.bottom-nav,.cal,.salary-net,.salary-metrics,.rain-station,.precip-visual,.forecast-strip,.insight-copy,.modal-sheet,.wx-detail-sheet,.screen-heading,.schedule-strip,.account-card,.quick-controls,.nature-controls')].map(e=>e.getBoundingClientRect()).filter(r=>r.bottom>0&&r.top<h);
    }
    function vignetteAlpha(x,y){return clamp(Math.abs(x/w-.5)*2,.1,1)*(y>h-110?.25:1)}
    ...
    function drawSky(m,hour,dark){
      ...
      if(!night&&!wet){
        const solar=clamp((hour-6)/12,0,1),sx=w*(.18+.64*solar),sy=32+Math.sin(Math.PI*solar)*-32;
        const warm=hour<8||hour>16,alpha=cover?.075:.24;
        glow(bg,sx,sy,Math.max(210,w*.65),warm?'249,181,97':'255,236,173',alpha);
        if(!cover){
          bg.save();bg.translate(sx,sy);bg.rotate(Math.sin(time*.015)*.055);
          for(let i=0;i<5;i++){
            const angle=.22+i*.52,len=h*.82,g=bg.createLinearGradient(0,0,Math.sin(angle)*len,Math.cos(angle)*len);
            g.addColorStop(0,'rgba(255,231,174,.09)');g.addColorStop(1,'rgba(255,231,174,0)');bg.fillStyle=g;bg.beginPath();bg.moveTo(-2,0);bg.lineTo(Math.sin(angle-.055)*len,Math.cos(angle-.055)*len);bg.lineTo(Math.sin(angle+.055)*len,Math.cos(angle+.055)*len);bg.fill();
          }bg.restore();
          glow(bg,sx,sy,48,warm?'255,206,139':'255,248,214',.32);
        }
      }
      if(night&&!cover){
      ...
      const count=cover?3:1;
      for(let i=0;i<count;i++){
        const size=Math.min(660,w*(.85+i*.25)),x=((time*(1.3+i*.45)+i*w*.7)%(w+size))-size;
        const y=12+i*70;
        imageDraw(bg,images.cloud[i%3],x,y-size*.29,size,size,wet?.12:night?.065:.19);
      }
      if(wet||m==='fog'||m==='cold'){
    ...
    function drawLeaves(dt,season,m){
      ...
      const count=level==='subtle'?3:level==='rich'?9:6;
      for(let i=0;i<count;i++){
        ...
        const side=i%2?-1:1,cx=i%2?w-18:18;
        const x=cx+side*(Math.sin(p.phase)*30+16),y=p.y*h;
        const size=(season===0?17:24)+18*p.z,flip=Math.cos(p.phase*.72);
        ...
        imageDraw(ctx,images[season===0?'petal':'leaf'][i%(season===0?3:4)],-size/2,-size/2,size,size,(.45+.28*p.z)*vignetteAlpha(x,y));ctx.restore();
      }
    }
    function drawRain(dt,m,dark){
      if(!allowed('weather')||!rainMode())return;
      const heavy=m!=='rain',count=level==='subtle'?26:level==='rich'?110:heavy?82:48;
      ...
      for(let i=0;i<count;i++){
        ...
        const x=p.x*w,y=p.y*h,len=5+20*p.z,edge=vignetteAlpha(x,y),a=(dark?.2:.24)*(.4+p.z)*edge;
        ctx.strokeStyle=dark?`rgba(207,232,240,${a})`:`rgba(77,119,143,${a})`;ctx.lineWidth=.4+p.z*.8;ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(x+len*tilt,y+len);ctx.stroke();
      ...
      // A few stationary glass droplets, restricted to the margins.
      for(let i=0;i<6;i++){
        const x=i%2?w-8-((i*7)%22):9+((i*11)%20),y=95+(i*127)%(h-190),r=2.2+i*.4;
        ctx.strokeStyle='rgba(119,166,179,.32)';ctx.lineWidth=.7;ctx.beginPath();ctx.ellipse(x,y,r,r*1.3,.15,0,Math.PI*2);ctx.stroke();ctx.strokeStyle='rgba(255,255,255,.7)';ctx.beginPath();ctx.arc(x-.4,y-.6,r*.65,3.4,5.1);ctx.stroke();
      }
    ...
    function draw(dt){
      ...
      // Keep all primary reading/interaction areas clear of foreground decoration.
      if(time-rectTick>.6||rectTick===0){collectQuiet();rectTick=time||.0001}
      ctx.save();ctx.globalCompositeOperation='destination-out';ctx.fillStyle='#000';for(const r of quiet)ctx.fillRect(r.left-9,r.top-7,r.width+18,r.height+14);ctx.restore();
    }
    function tick(now){
    ...
    return{update,getMode:()=>mode,refresh:resume,setQuality,getQuality:()=>level,preview:previewMode,_forceSilence(){if(root.WxSfx)root.WxSfx._forceSilence()},_debug:()=>({mode,active:activeMode(),frames:frameCount,running:!!raf,quality:level,reduced:reduced.matches,canvasCount:document.querySelectorAll('#wxfx').length})};
  }};
})(window);
